...
class SeasonTile extends React.Component {
  render() {
    ...
    return (
      <Link to={`/shows/${this.props.showId}/${this.props.seasonNumber}`} className="bg-white block pb-3 rounded-md shadow-md hover:bg-gray-100 transition-colors duration-200">
        {posterUrl
        ...
        <span className="block text-center text-gray-500 text-sm">{aired && `(${aired})` }</span>
      </Link>
    );
  }
}
